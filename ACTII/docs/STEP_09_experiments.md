...
def variant_b_prompt(history: list[dict], delta: str, ratio: float, goal: str) -> str:
    ...
    Budget = 50 (trauma) + checkpoint_budget(r) + 100 (delta).
    No protected layer — everything goes through one compression pass.
    """
...
def run():
    ...
    for ratio in RATIOS:
        ...
        for sample in samples:
            ...
            # ratio passed at init so Echidna uses it from the first checkpoint
...
|---|---|---|
| Variant A F1 > B at lower r | **Pass** — protected buffer works | Lead with this in paper |
| Variant A ≈ Variant B | Inconclusive | Report honestly; lean on token curve + revert |
| Variant B F1 > Variant A | **Fail** | Concede §3.5; do not overclaim |

---

## Done when

- `microclaim.csv` has 5 rows with both F1 columns populated
- `turn15_probe.txt` shows both answers and F1 scores
- Results reported exactly as measured
