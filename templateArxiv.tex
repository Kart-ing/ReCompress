...
2{,}166 vs 2{,}960), and by 10 turns it is less than half (3{,}499 vs 7{,}700), the gap
widening because rule-trigger overhead grows roughly linearly while uncached naive cost grows
super-linearly (Figure~\ref{fig:crossover}). The earlier honest caveat still holds: against a
...
Note the answer-quality columns are within the $n=20$ noise band of Section~\ref{sec:audits}
(no CI); we read the rule trigger as quality-neutral, not better or worse.

\begin{figure}[!t]
  \centering
  \includegraphics[width=0.92\linewidth]{media/crossover.png}
  \caption{Total tokens (context $+$ compression overhead) vs.\ conversation length. With the
  free rule-based Echidna (blue), RbD-Compress falls below the uncached naive agent (red) by 6
  turns and the gap widens; the LLM-Echidna variant (grey) is dominated. A KV/prefix-cached
  naive deployment (dashed) stays lowest --- caching is the equalizer we do not beat on raw
  tokens. Measured at $T=6,10$; the slopes extrapolate the trend.}
  \label{fig:crossover}
\end{figure}
