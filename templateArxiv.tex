...
we report honestly that, once per-turn compression overhead is counted, this is
a win only when the context window (not total token spend) is the binding
constraint, or when the solver is far more expensive than the compressor. The
flat-context property and the protected-buffer mechanism are the contributions;
a total-token win at realistic horizons remains future work.
