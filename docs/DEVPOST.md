...
- **We measured our multi-turn overhead honestly — and found our own expensive component was useless.** The "8.1× flatter context" is the solver-context axis; counting the per-turn compression LLM calls, the system actually costs *more* in total tokens at short horizons. Digging in, the LLM "Echidna" checkpoint-trigger turned out to decide `checkpoint` **98.3% of the time** — no real decision. We replaced it with a free rule (2.6× cheaper, same F1), which makes the system beat an *uncached* growing-history agent on total tokens from ~6 turns. Against a *cached* agent it still doesn't win on raw tokens — and we say so.
