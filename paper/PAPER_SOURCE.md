...
Context compression for LLMs is dominated by *extractive* methods that delete or select tokens (e.g. The Token Company's bear-1.1, LLMLingua). We argue and show that extraction has a **structural ceiling**: when an answer depends on facts spread across many individually-low-salience tokens, no in-budget subsequence preserves it, but a paraphrase does — so *abstractive, query-aware rewriting* is necessary, not merely better. We distill a query-aware compressor (DeepSeek teacher → **Qwen2.5-1.5B + LoRA**) and benchmark it head-to-head against bear-1.1 at a **matched token budget** with a paired 5-bar design and bootstrap 95% CIs across four QA datasets. The distilled 1.5B beats bear by **+0.252 F1 on HotpotQA (+56%, CI excludes 0)** and **+0.180 on 2WikiMultihopQA (+46%)** — the latter a dataset it never trained on (zero-shot generalization) — recovering ~64% of the frontier teacher's margin while running offline for ~cents. We then apply the same compressor as the memory engine of a multi-turn agent (Re:Zero), keeping context **flat (~184 tokens) where a naive agent grows to 1,482 over 12 turns (8.1×)** at higher answer quality. We report the full trajectory including negative results: stacking with deletion is a wash-to-worse, answer-grounded distillation underperforms teacher-imitation and hurts out-of-distribution generalization, and on abstractive free-form QA (MS MARCO) the method ties bear — scoping the win to the multi-hop, distractor-heavy, entity-answer regime that blind deletion cedes. Total compute: ~$10.
...
**Positive:**
1. **Headline:** distilled Qwen2.5-1.5B (Hearth) beats bear at matched budget — HotpotQA **+0.252 (+56%), CI (0.103,0.396)**.
2. **Cross-dataset generalization (strongest evidence):** trained ONLY on HotpotQA-derived data, yet 2Wiki **+0.180 (+46%), CI (0.030,0.340)** zero-shot. "the 2Wiki PASS is the strongest evidence the model learned query-aware compression as a *skill*, not HotpotQA trivia."
...
5. **No LLMLingua head-to-head — did NOT beat published SOTA.** "We beat bear-1.1, the sponsor's product, the relevant comparison for this track." Distinction is methodological (delete vs rewrite).
